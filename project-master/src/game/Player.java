...
import game.actions.TokenDropAction;
...
public class Player extends Actor implements Soul, Resettable {
	...
	public Action playTurn(Actions actions, Action lastAction, GameMap map, Display display) {
		if (this.hasCapability(Status.DISARMED)) {
			this.removeCapability(Status.HOSTILE_TO_ENEMY);
		}
		// Place the Token On Map
		if (this.hasCapability(Status.RESET)) {
			if (this.hasCapability(Status.VALLEY_DIE)) {
				this.removeCapability(Status.VALLEY_DIE);
				new TokenDropAction(this.previousPlace).execute(this,map);
			}else if(!this.hasCapability(Status.REST)) {
				new TokenDropAction(map.locationOf(this)).execute(this,map);
			}else{
				this.removeCapability(Status.REST);
			}
